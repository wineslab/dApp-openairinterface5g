...
#ifdef E3_AGENT
  /* Same k2: sort by ascending start symbol (then descending length) so the
   * TDAs that fit a slot form a contiguous suffix, which get_num_ul_tda()
   * relies on when it takes the first fitting TDA. */
  if (as < bs)
    return -1; // earlier start first
  if (as > bs)
    return 1;
#endif /* E3_AGENT */
...
void nr_rrc_config_ul_tda(NR_ServingCellConfigCommon_t *scc, int min_fb_delay, int do_SRS,
                          int num_additional_ul_tdas, const additional_ul_tda_t *additional_ul_tdas)
{
  NR_PUSCH_TimeDomainResourceAllocationList_t *tda_list =
      scc->uplinkConfigCommon->initialUplinkBWP->pusch_ConfigCommon->choice.setup->pusch_TimeDomainAllocationList;
  ...
  if (num_additional_ul_tdas > 0) {
    ...
    for (int i = 0; i < tda_list->list.count; i++) {
      ...
      for (int j = 0; j < n_k2; j++)
        if (k2_values[j] == k2v) { found = true; break; }
      if (!found)
        k2_values[n_k2++] = k2v;
    }
    ...
    AssertFatal(tda_list->list.count + needed <= 16,
                ...
                num_additional_ul_tdas, n_k2, needed,
                16 - tda_list->list.count, tda_list->list.count);
    ...
    for (int i = 0; i < num_additional_ul_tdas; i++) {
      ...
      for (int j = 0; j < n_k2; j++) {
        ...
        for (int e = 0; e < tda_list->list.count; e++) {
          if (tda_list->list.array[e]->k2 != NULL
              && *tda_list->list.array[e]->k2 == k2_values[j]
