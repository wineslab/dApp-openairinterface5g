...
#ifdef E3_AGENT
  /* Hand each UL/MIXED slot's IQ to the L1-KPM SM (RF=2) via /e3_ran_buffers.
   * Only when a dApp is subscribed */
  int sel = nr_slot_select(&gNB->gNB_config, frame_rx, slot_rx);
  if ((sel == NR_UPLINK_SLOT || sel == NR_MIXED_SLOT) && l1_kpm_sm_has_subscribers()) {
